...
test.describe("Elements - Dynamic Properties page", () => {
  test("Button becomes enabled after 5 seconds", async ({dynamicPropertiesPage}) => {
  ...
  test("Button color changes", async ({dynamicPropertiesPage}) => {
    const { initialColor, finalColor } = await dynamicPropertiesPage.verifyButtonColorChange();
    console.log(`Color changed from ${initialColor} to ${finalColor}`);
